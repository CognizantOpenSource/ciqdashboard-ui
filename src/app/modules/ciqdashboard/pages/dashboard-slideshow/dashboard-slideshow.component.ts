...
import { map, distinctUntilChanged, take, filter, distinct } from 'rxjs/operators';
...
export class DashboardSlideshowComponent extends IDashBoard implements OnInit {
  ...
  ngOnInit() {
    ...
    this.managed(this.route.queryParams).pipe(map(p => {
      ...
      this.managed(this.route.params.pipe(map(params => params.dashboardId), distinctUntilChanged()))
        .subscribe(id => this.dashboardService.loadDashboard(id, categoryType));
  ...
  private loadItems(dashboard, page = 0) {
    ...
    if (page >= 0 && dashboard) {
      ...
      this.currentSlide = 0;
      this.updateItemData(this.slides[this.currentSlide], this.page, this.currentSlide, dashboard.name, dashboard.projectName);
    }
  }
  private navChange() {
    const item = this.slides[this.currentSlide];
    if (item && !item.data) {
      this.updateItemData(item, this.page, this.currentSlide, this.dashboard.name, this.dashboard.projectName);
    }
  }
  previous() {
