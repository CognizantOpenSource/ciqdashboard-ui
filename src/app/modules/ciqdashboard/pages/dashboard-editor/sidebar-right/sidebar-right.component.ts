...
import { distinctUntilChanged, map,distinct } from 'rxjs/operators';
/**
...
export class SidebarRightComponent extends UnSubscribable implements OnInit {
  ...
  ngOnInit() {
    ...
    this.managed(this.route.queryParams).pipe(map(p => {
      //console.log('rightside',p);
      return p.category
    }), distinctUntilChanged()).subscribe(category => {
      this.category = category;
      //console.log('sidebar-category: ' + this.category);
    });
  }
  ...
  editItem(item) {
    ...
    const queryParams = {
      ...
      ,returnUrl: this.router.routerState.snapshot.url,category:this.category
    }
    const component = itemGroup === 'dataimg' ? 'edit-img' : (itemGroup === 'datalabel' ? 'edit-label' : 'edit-chart');
    if (projectId) {
      //console.log('editcharts',projectId,component,queryParams);
