...
import { DashboardItemsService, clean } from '../../services/ciqdashboard-items.service';
import { ToastrService } from 'ngx-toastr';
import { parseApiError } from 'src/app/components/util/error.util';
import { DashboardDataSourceService } from "../../services/ciqdashboard-datasource.service";
...
export class CreateItemComponent extends BaseItemEditor implements OnInit {
  ...
  ngOnInit() {
    ...
    //console.log('optionscreate',this.options);
    ...
    this.managed(this.route.queryParams).subscribe(q => { 
      //console.log('queryParams',q,this.route.queryParams,this.route);
    ...
    this.route.queryParams.pipe(take(1)).subscribe(q => {
      ...
      if (this.createMode) {
        ...
        this.dashItemService.getItem(q.itemId).subscribe(item => {
          this.item = { ...item };
          //console.log('CreateChartitemss',this.item);
  ...
  reload() {
    ...
    //console.log('itemOptions',this.item.options);
    ...
    this.dashItemService.previewChartItem(this.selectAggregationType({ ...this.item })).subscribe(item => {
      ...
      //console.log('itemdata',this.item._data);
    }, error => {
  ...
  onlookupsCollectionSelected(collectionSelected: string) {
    ...
    this.datasoruceService.getFieldsTypes(collectionSelected).subscribe((collectionFields) => {
      ...
      let finalfields = getCollectionField.filter(ele => !ele.name.startsWith('_'));
      this.ItemslookupCol.push(finalfields);
