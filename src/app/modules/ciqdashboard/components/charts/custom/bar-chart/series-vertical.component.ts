...
@Component({
    selector: 'g[ngx-charts-series-vertical2]',
    template: `
    <svg:g
      ngx-charts-bar
    ...
    <svg:g *ngIf="showGauge">
      <svg:g
        ngx-charts-bar-gauge
...
export class SeriesVerticalComponent2 implements OnChanges {
    @Input() dims;
    @Input() type = 'standard';
    @Input() series;
    @Input() xScale;
    ...
    barsForDataLabels: Array<{ x: number; y: number; width: number; height: number;
         total: number; target: number; series: string ; name: string ; color:string;}> = [];
    ...
    updateDataLabels() {
        if (this.type === 'stacked') {
            ...
            this.barsForDataLabels = this.series.map(d => {
                ...
                section.target = d.target || (d.extra && d.extra.target);
                section.name =  this.seriesName ? d.name : undefined;
                section.color = this.colors.getColor(d.name);
