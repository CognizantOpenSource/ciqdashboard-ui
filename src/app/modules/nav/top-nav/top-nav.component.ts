...
export class TopNavComponent implements OnInit {
  ...
  constructor(
    ...
    this.orgs$ = projectService.orgs$;
    this.lobs$ = projectService.lobs$;
  }

  ngOnInit() {
    this.auth.state$.pipe(filter(auth => !!auth), tap(() => this.loggedIn = true)).subscribe(() => { this.projectService.loadProjects(); this.projectService.loadORGs(); });
    this.theme$ = this.config.userSettings$.pipe(map(it => (it.theme) || Theme.default));
    this.projectService.project$.subscribe(project => {
      this.project = project;
    });
    this.projectService.org$.subscribe(org => {
      this.org = org;
    });
    this.projectService.lob$.subscribe(lob => {
      this.lob = lob;
    });
    this.router.events.pipe(filter(event => event instanceof NavigationEnd), map(() => this.route.snapshot))
      .subscribe((state) => {
        ...
        this.orgLoaded = !!state.params.orgId;
        this.lobLoaded = !!state.params.lobId;
      }
      );
  }
  get showFullHeader$() {
