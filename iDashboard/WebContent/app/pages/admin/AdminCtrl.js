...
(function() {
	...
	angular.module('MetricsPortal.pages.admin').controller('AdminCtrl',
			AdminCtrl).directive(
			"keepScroll",
			function() {
				return {
	...
	function AdminCtrl($filter, AES, $sessionStorage, $scope, $http, $sce,
							...
							function(response) {
								if (response > 0) {
									...
									$uibModal
											.open({
												animation : true,
												templateUrl : 'app/pages/admin/deleteTemplateName.html',
					...
					.success(
							function(response) {
								...
								if (response == 0) {
									$scope
											.open(
													'app/pages/admin/templateUpdateSuccessfully.html',
													'sm');
					...
					.success(
							function(response) {
								...
								if ($scope.numberOfTemplate <= $scope.templateLength) {
									$scope
											.open(
													'app/pages/admin/restrictTemplateCreationMessage.html',
													'sm');
		...
		$scope.updateUser = function(details) {
			...
			$uibModal.open({
				animation : true,
				templateUrl : 'app/pages/admin/updateAdminUser.html',
				scope : $scope,
				size : 'sm',
				resolve : {
					items : function() {
		...
		$scope.editUser = function(userDetails) {
			...
			$uibModal.open({
				animation : true,
				templateUrl : 'app/pages/admin/editUser.html',
		...
		$scope.creatingUser = function(userDetails) {
			...
			$uibModal.open({
				animation : true,
				templateUrl : 'app/pages/admin/creatingUser.html',
		...
		$scope.deleteUser = function(userDetails) {
			...
			$uibModal.open({
				animation : true,
				templateUrl : 'app/pages/admin/deleteUser.html',
		...
		$scope.deleteUserInfo = function(loginRequests) {
			...
			$http({
				url : "./rest/jsonServices/deleteUserList",
				method : "POST",
				data : data,
				headers : {
					'Authorization' : token
		...
		$scope.createAdminUser = function() {
			$uibModal.open({
				animation : true,
				templateUrl : 'app/pages/admin/createAdminUser.html',
				scope : $scope,
				size : 'sm',
				resolve : {
					items : function() {
		...
		$scope.createOrg = function(getid) {
			...
			$uibModal.open({
				animation : true,
				templateUrl : 'app/pages/admin/createOrg.html',
				scope : $scope,
				size : 'sm',
				resolve : {
					items : function() {
		...
		$scope.showSuccessMsg = function() {
			$scope.open('app/pages/admin/savedSuccessfully.html', 'sm');
			setTimeout(function() {
				$scope.reload();
			}, 1000);
		...
		$scope.getSelection = function(userId) {
			...
			$http({
				...
				if (response == 0) {
					...
					$scope.open('app/pages/admin/updatedAdminMsg.html', 'sm');
				}
			});
			setTimeout(function() {
				$state.reload();
