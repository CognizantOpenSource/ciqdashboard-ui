...
(function() {
	...
	angular.module('MetricsPortal.Operational').controller(
	...
	function OperationalDashboardCtrl($sessionStorage, AES, $base64, $element,
		...
		$scope.expandCompress = function(nam) {
			if (eval(nam).style.display == "") {
				...
				eval("Mod" + nam).src = "app/Operational/Home/asset/collapse.png";
			} else if (eval(nam).style.display == "none") {
				eval(nam).style.display = "";
				eval("Mod" + nam).src = "app/Operational/Home/asset/expand.png";
			}
		};
		...
		$scope.expandCompressJira = function(nam) {
			if (eval(nam).style.display == "") {
				eval(nam).style.display = "none";
				eval("JiraMod" + nam).src = "app/Operational/Home/asset/collapse.png";
			} else if (eval(nam).style.display == "none") {
				eval(nam).style.display = "";
				eval("JiraMod" + nam).src = "app/Operational/Home/asset/expand.png";
			}
		};
		...
		$scope.openDashboard = function(item) {
			...
			if (item.releaseSet.length == 0 || item.templateName == '') {
				$scope.open('app/Operational/Home/alerts/noAccess.html', 'sm');
			} else {
		...
		$scope.saveDashboard = function(form, selectedtemplate) {
			...
			if (selectedItems == "" && selectedJiraItems == "") {
				$scope.open(
						'app/Operational/Home/alerts/selectDashboardMsg.html',
						'sm');
			} else {
						...
						.success(
								function(response) {
									...
									if (response == 0) {
										$scope
												.open(
														'app/Operational/Home/alerts/saveDashboardMsg.html',
														'sm');
									} else {
										$scope
												.open(
														'app/Operational/Home/alerts/existDashboardMsg.html',
														'sm');
									}
								});
				setTimeout(function() {
						...
						.success(
								function(response) {
									...
									if (response == 0) {
										...
										$scope
												.open(
														'app/Operational/Home/alerts/updatedDashboardMsg.html',
														'sm');
									} else {
		...
		$scope.deleteDashboard = function(dashid) {
			...
			$uibModal
					.open({
						animation : true,
						templateUrl : 'app/Operational/Home/alerts/deleteDashboard.html',
						scope : $scope,
						size : 'sm',
						resolve : {
							items : function() {
